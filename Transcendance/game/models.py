...
class Tournament(models.Model):
	...
	def create_next_round(self):
		...
		if self.current_round == 0: #Cas du premier round
			...
			if plays_from_last_round.count() == 1:#La finale a ete jouee
				...
				#Lancement dans un thread separe car pas de boucle d'evenement ici puisque contexte syncrhone (asyncio.create_task impossible)
				threading.Thread(target=async_to_sync(self.store_score_on_blockchain)).start()
				#Stockage BLOCKCHAIN
				# asyncio.create_task(self.store_score_on_blockchain())#A voir si je dois pas save ici
	...
	async def store_score_on_blockchain(self):
		try :
			...
			print(f'Mauvaise donnees transmises au smart contract : {e}')
		except exceptions.ContractLogicError:
			print(f'L interraction ne respecte pas les regles imposees par le smart contract : {e}')
		except exceptions.BadFunctionCallOutput as e:
			print(f'Erreur de retour du smart contract : {e}')
		except exceptions.TransactionNotFound as e:
			print(f'La transaction attendu n existe pas sur la Blockchain : {e}')
		except exceptions.InvalidAddress as e:
			print(f'L adresse utilisee est invalide : {e}')
		except Exception as e:
			print(f'Une erreur est survenue lors du stockage du score sur la blockchain: {e}')
