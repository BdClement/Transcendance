...
class TestPlayConsumer(LiveServerTestCase):
	...
	async def test_can_connect_and_disconnect(self):
		communicator = WebsocketCommunicator(application, f"/ws/game/{self.play_remote.id}/")
	...
	async def test_bad_game_id_connection(self):
		communicator = WebsocketCommunicator(application, "/ws/game/1000/")
		connected = await communicator.connect()
	...
	async def test_connection_play_unavailable(self):
		...
		communicator2 = WebsocketCommunicator(application, f"/ws/game/{self.play_unavailable.id}/")
	...
	async def test_integration_Pong(self):
		...
		communicator = WebsocketCommunicator(application, f"/ws/game/{self.play.id}/")
		connected = await communicator.connect()
		self.assertTrue(connected)
